---
title: "Overview of flow based MCMC"
author: "Kiize"
date: 2026-01-13
...
$$
with $S$ a [quartic action](quartic_action.qmd).

Having said that, we construct the map $f$ using the real non-volume-preserving (NVP) flow approach, that is we construct $f$ by composition of [affine coupling](affine_couplings.qmd) layers $\{g_{i}\}$
$$
f(\phi) = g_{1} \circ g_{2} \circ \dots g_{n}(\phi).
$$

The proposal distribution $\tilde{p}_{f}$ is then given by
$$
\tilde{p}_{f}(\phi) = r(f(\phi)) \left|\det \frac{ \partial f(\phi) }{ \partial \phi }  \right|.
$$
Affine couplings also solve the problem of computing the jacobian.